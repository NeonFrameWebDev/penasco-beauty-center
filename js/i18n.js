...
(function () {
  ...
  function applyLang(lang) {
    ...
    document.querySelectorAll(".lang-toggle").forEach((btn) => {
      ...
      // Update flag display
      const flagEl = btn.querySelector(".lt-flag");
      const labelEl = btn.querySelector(".lt-label");
      if (lang === "es") {
        // Currently Spanish; show EN to switch to English
        if (flagEl) flagEl.textContent = "🇺🇸";
        if (labelEl) labelEl.textContent = "EN";
      } else {
        // Currently English; show MX flag to switch to Spanish
        if (flagEl) flagEl.textContent = "🇲🇽";
        if (labelEl) labelEl.textContent = "ES";
      }
    });
